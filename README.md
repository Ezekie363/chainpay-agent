...
You: BTC 和 ETH 的价格分别是多少？顺便查一下 gas
  ...
  ✔ Token Price  (19,428 ms)  −$0.01 USDC  ETH = $1,571.87 (+3.29%)
  ✔ Token Price  (19,479 ms)  −$0.01 USDC  BTC = $61,077.00 (+1.14%)
  ✔ Gas Price    (19,561 ms)  −$0.01 USDC  0.01 gwei

  本轮：3 次查询  ·  支付 $0.03 USDC  ·  会话累计 3 次  $0.03 USDC

You: 查询 Aave 和 Uniswap 的 TVL

  ✔ DeFi TVL  (18,102 ms)  −$0.01 USDC  AAVE TVL = $23.45B
  ✔ DeFi TVL  (18,340 ms)  −$0.01 USDC  UNISWAP TVL = $4.12B

You: 当前市场恐惧贪婪指数

  ✔ Fear & Greed  (18,303 ms)  −$0.01 USDC  12 — Extreme Fear
```

USDC 扣款真实发生在 Base Sepolia 上，每笔附带 tx hash，可在链上查验。
...
| 文件 | 职责 |
|------|------|
| `agent/agent.py` | DeepSeek LLM 工具调用循环，支付事件跟踪，6 轮对话历史管理 |
| `agent/tools/caw_signer.py` | Cobo MPC `message_sign` 封装，实现 x402 的 `ClientEvmSigner` 接口 |
| `agent/tools/chain_query.py` | 6 个付费查询工具，通过 x402 httpx 客户端调用 |
| `server/main.py` | FastAPI 服务端，内置 x402 收费中间件，返回真实市场与链上数据 |
| `main.py` | 富文本终端 UI，实时显示支付流水、涨跌幅和会话摘要 |
...
| 工具 | 说明 |
|------|------|
| 代币价格 | 37 种代币实时美元价格 + 24h 涨跌幅 |
| Gas 价格 | Base Sepolia 基础费 / 优先费 / 综合价格 |
| ETH 余额 | 指定地址 ETH 余额及美元估值 |
| USDC 余额 | 指定地址 Base Sepolia USDC 余额 |
| DeFi TVL | 12 个主流协议锁仓量（Uniswap、Aave、Lido 等） |
| 恐惧贪婪指数 | 全市场情绪指数 0-100，含文字分类 |

### 支持的代币（37 种）

L1 主链：ETH、BTC、SOL、BNB、AVAX、DOT、ADA、XRP、ATOM、NEAR、APT、SUI、INJ、SEI  
L2 / Rollup：MATIC、ARB、OP、STRK  
稳定币：USDC、USDT、DAI、ENA  
Wrapped / LST：WBTC、STETH  
DeFi：LINK、UNI、AAVE、CRV、GMX、PENDLE  
Meme：DOGE、SHIB、PEPE、WIF  
AI：WLD  

### 支持的 DeFi 协议（TVL 查询）

Uniswap、Aave、MakerDAO、Lido、Compound、Curve、PancakeSwap、GMX、dYdX、Pendle、Morpho、Sky
...
每轮对话结果写入会话历史，最多保留最近 6 轮。追问已有结果或纯概念问题时，LLM 直接从历史回答，不触发新工具调用，避免不必要的支付。

### 会话预算上限

每次会话设有 **0.05 USDC** 支出上限（约 5 次查询）。Agent 在每次工具调用前检查剩余额度，超限时主动告知用户而非静默失败，重启 Agent 后预算重置。上限可通过 `main.py` 中的 `SESSION_BUDGET_USDC` 常量调整。
...
| 数据类型 | 来源 |
|----------|------|
| 代币价格 / 涨跌幅 | CoinGecko 免费 API（37 种代币） |
| Gas 价格 | Base Sepolia 公共 RPC（`eth_gasPrice`、`eth_getBlock`） |
| ETH / USDC 余额 | Base Sepolia 公共 RPC |
| DeFi TVL | DeFiLlama API（免费，无需 Key） |
| 恐惧贪婪指数 | Alternative.me API（免费，无需 Key） |
...
| 接口 | 数据 |
|------|------|
| `GET /api/token-price/{symbol}` | 代币美元价格 + 24h 涨跌幅 |
...
| `GET /api/defi-tvl/{protocol}` | DeFi 协议锁仓量（TVL） |
| `GET /api/fear-greed` | 市场恐惧贪婪指数（0-100） |
...
输出包含付费查询的实时流水，以及会话前后的 USDC 余额对比。
